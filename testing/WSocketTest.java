...
public class WSocketTest {
	...
	/**
	 * runs a simple echo server also shows how to check CRC checksums on server
	 * side
	 */
	...
	/**
	 * This method can be called from a background thread that handles network
	 * interactions with the server. This way, the GUI thread would not be
	 * blocked.
	 */
	...
	public static void userPressedVoteButton() {
		try {
			...
			for (int i = 0; i < 10000; i++)
				big_data.add(new Vote(new Voter("Ronald", "McDonald",
						new Address(), 199299399), new Candidate("George",
						"Bush", new Address(), 616717818)));

			final Message req = new Message(Message.Method.GET, "test", "test");
			...
			new Thread(new Runnable() {
				...
				public void run() {
					try {
						// will take a while to process with extremely large
						// data sets
