...
public class RegistrationPanel extends JPanel {
	...
	public RegistrationPanel(District district) {
		...
		provincesComboBox = Utilities.newJComboBox(Province.getProvinceList(),
				1, 8, layout);
	...
	public boolean validateFields() {
		...
		if (!firstNameText.getText().isEmpty()
				&& firstNameText.getText().matches("^[a-zA-Z]+$")) {
		...
		if (!streetNumberText.getText().isEmpty()
				&& streetNumberText.getText().matches("^[0-9]+$")) {
		...
		if (!streetText.getText().isEmpty()
				&& streetText.getText().matches("^[a-zA-Z\\s]+$")) {
		...
		if (!cityText.getText().isEmpty()
				&& cityText.getText().matches("^[a-zA-Z\\s]+$")) {
		...
		if (postalCodeText.getText().matches(
	...
	public boolean registerUser() {
		if (ClientController.registerUser(new Voter(firstNameText.getText(),
				lastNameText.getText(), new Address(streetNumberText.getText()
						+ " " + streetText.getText(), cityText.getText(),
						Province.getProvinceFromName(provincesComboBox
								.getSelectedItem().toString()), postalCodeText
								.getText()),
				Integer.parseInt(sinText.getText()), new String(passwordText
						.getPassword()), district))) {
			clearFields();
			return true;
		} else {
			sinText.setForeground(ERROR_COLOUR);
