...
public class WSocket
{
    ...
    throws IOException {
        ...
        if ( msg.getType() == Message.Type.FRAGMENT_START)
    ...
    throws IOException {
        ...
        if ( msg.getType() != Message.Type.FRAGMENT && data.length > PACKET_LEN ) {
    ...
    throws IOException { 
         ...
         while ( !done_receiving ) {
             ...
             if ( msg.getType() == Message.Type.FRAGMENT) {
                 byte[] frag = (byte[])msg.getData();
                 for ( int i=0; i<frag.length; i++ )
                     total_data.add(frag[i]);
             } else if ( msg.getType() == Message.Type.FRAGMENT_DONE) {
                 done_receiving = true;
             } else { 
